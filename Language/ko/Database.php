...
return [
   'invalidEvent'                     => '{0}의 경우는 유효하지 않은 Model Event callback입니다.', //  CI4 rc.1기준 : {0}에는 method가 들어감. '은, 는, 이, 가' 조사를 붙이기가 애매해서 '{0}의 경우는'라고 덧붙임. // '{0} is not a valid Model Event callback.',
   'invalidArgument'                  => '{0} 대신 반드시 유효한 인수가 제공되어야 합니다.', //  CI4 rc.1기준 : {0}에는 argument가 들어감. // 'You must provide a valid {0}.',
   'invalidAllowedFields'             => 'Model을 위한 fileds는 반드시 명시가 되어야 합니다.: {0}', // 'Allowed fields must be specified for model: {0}',
   'emptyDataset'                     => '{0} : data가 들어있지 않습니다.', // CI4 rc.1기준 : DB 작업 관련, data가 없는 경우. {0}에는 'insert', 'update', 'chunk'가 들어감. // 'There is no data to {0}.',
   ...
   'parseStringFail'                  => 'key string 구문 분석을 실패함.', // CI4 rc.1기준 : 'parseStringFail'를 사용하는 곳을 찾지 못했음. 차후 CI 팀에 의해서 변경될 수도 있을 듯함. 차후, 사용되는 구문에 따라서 해석이 변경될 수 있음. // 'Parsing key string failed.',
   ...
   'forEmptyInputGiven'               => '필드에 `{0}` 경우를 사용하기 위해서 비어있는 구문이 지정되었습니다.', // CI4 rc.1기준 : {0}에 string $argument 또는 'Select'가 들어감. // 'Empty statement is given for the field `{0}`',
